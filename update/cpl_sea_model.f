...
C     Input variables:
      ...
      real hfseacl(nlon,nlat)   ! ACCESS-CM2 0-40m advection (W m-2)
...
C     Net heat flux: online atmosphere plus prescribed CM2 advection
      hflux(:,:) = hfsea(:,:) + hfseacl(:,:)

      sstd(:,:) = rhcaps(:,:)*hflux(:,:)

C     Do not also apply idealized diffusion: CM2 advection already
C     represents resolved horizontal and vertical ocean heat transport.
c      call HORDIFSEA(sst0,sstd)
