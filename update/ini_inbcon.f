...
C--   3.3 ACCESS-CM2 0-40 m ocean-advection forcing
C     Twelve direct-access big-endian REAL*4 records in W m-2.
      if (iitest.ge.1) print*,' reading CM2 ocean advection'
      irecl = 4*ix*il
      open (unit=32,file='fort.32',status='old',form='unformatted',
     &      access='direct',recl=irecl)
      do it=1,months
        read (32,rec=it) r4inp
        do j=1,il
          do i=1,ix
            qadvmnth(i,j,it)=r4inp(i,j)
          enddo
        enddo
      enddo
      close (32)
C     FORCHK sets CM2 advection to zero at SPEEDY land points.
      CALL FORCHK (bmask_s,qadvmnth,ix*il,months,-2000.,2000.,0.)

C--   3.4 SST anomalies for initial and prec./following months
